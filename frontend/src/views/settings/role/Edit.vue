    ...
    methods: {
        ...
        viewRole() {
            this.$store.dispatch('rolesAndPermissions/view', this.$route.params.id).then(response => {
                this.role.form.name = response.data.data.data.name
                this.role.form.is_core = response.data.data.data.is_core === 1
                this.checkPermissions(response.data.data.data.permissions)
                this.role.isCardLoading = false
            }).catch(error => {
                console.error(error)
                this.role.isCardLoading = false
