                    ...
                    >
                        ...
                        >
                            <validation-observer
                                ...
                                v-slot="{ handleSubmit }"
                            >
                                <b-form @submit.prevent="handleSubmit(editNews)">
                                    <b-row>
                                        <!-- Photo -->
                                        <b-col
                                            lg="6"
...
export default {
    ...
    methods: {
        ...
        viewNews() {
            this.news.isCardLoading = true
            this.$store.dispatch('news/view', this.$route.params.id).then(response => {
                this.news.form.title_en = response.data.data.data.title_en
                this.news.form.title_fr = response.data.data.data.title_fr
                this.news.form.details_en = response.data.data.data.details_en
                this.news.form.details_fr = response.data.data.data.details_fr
                this.news.form.site_id = response.data.data.data.site_id
                this.news.isCardLoading = false
            }).catch(error => {
                console.error(error)
                this.news.isCardLoading = false
        ...
        editNews() {
            ...
            Object.keys(this.news.form).forEach(key => {
                if (key === 'photo') {
                    if (this.news.form['photo']) data.append(key, this.news.form[key])
                } else {
            ...
            this.$store.dispatch('news/update', {
                ...
                this.$refs.editNewsForm.setErrors(error.response.data.errors)
                this.news.isLoading = false
            })
        },
        reset() {
