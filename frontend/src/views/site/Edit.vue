                    ...
                    >
                        ...
                        >
                            <validation-observer
                                ...
                                v-slot="{ handleSubmit }"
                            >
                                <b-form @submit.prevent="handleSubmit(editSite)">
                                    <b-row>
                                        <!-- Photo -->
                                        <b-col
                                            lg="6"
...
export default {
    ...
    methods: {
        viewSite() {
            this.site.isCardLoading = true
            this.$store.dispatch('site/view', this.$route.params.id).then(response => {
                this.site.form.name = response.data.data.data.name
                this.site.form.slug = response.data.data.data.slug
                this.site.form.about = response.data.data.data.about
                this.site.form.phone = response.data.data.data.phone
                this.site.form.email = response.data.data.data.email
                this.site.form.location_description = response.data.data.data.location_description
                this.site.form.location_url = response.data.data.data.location_url
                this.site.isCardLoading = false
            }).catch(error => {
                console.error(error)
                this.site.isCardLoading = false
            })
        ...
        editSite() {
            ...
            Object.keys(this.site.form).forEach(key => {
                if (key === 'photo' || key === 'catalog') {
                    if (this.site.form[key]) data.append(key, this.site.form[key])
                } else {
                    data.append(key, this.site.form[key])
                }
            })
            this.$store.dispatch('site/update', { id: this.$route.params.id, data }).then(response => {
                ...
                this.$refs.editSiteForm.setErrors(error.response.data.errors)
                this.site.isLoading = false
            })
        },
        reset() {
