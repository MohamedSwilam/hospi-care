        ...
        <b-container>
            <b-row>
                <b-col cols="12">
                    ...
                    >
                        <b-card-actions
                            ref="editProductDetail"
                            title="Add Product Detail"
                            action-collapse
                        >
                            <validation-observer
                                ref="editProductDetailForm"
                                v-slot="{ handleSubmit }"
                            >
                                <b-form @submit.prevent="handleSubmit(editProductDetail)">
                                    <b-row>
                                        <b-col
                                            lg="6"
                                            md="6"
...
export default {
    ...
    mounted() {
        this.product_detail.form.product_id = this.$route.params.product_id
        this.viewProductDetail()
    },
    methods: {
        viewProductDetail() {
            this.product_detail.isCardLoading = true
            this.$store.dispatch('productDetail/view', this.$route.params.detail_id).then(response => {
                this.product_detail.form.title_en = response.data.data.data.title_en
                this.product_detail.form.title_fr = response.data.data.data.title_fr
                this.product_detail.form.value_en = response.data.data.data.value_en
                this.product_detail.form.value_fr = response.data.data.data.value_fr
                this.product_detail.isCardLoading = false
            }).catch(error => {
                console.error(error)
                this.product_detail.isCardLoading = false
            })
        ...
        editProductDetail() {
            ...
            this.$store.dispatch('productDetail/update', {
                id: this.$route.params.detail_id,
                data: this.product_detail.form
            }).then(response => {
                ...
                this.$router.push(`/products/${this.$route.params.product_id}`)
            }).catch(error => {
                this.product_detail.isLoading = false
                this.$refs.editProductForm.setErrors(error.response.data.errors)
            })
        },
        reset() {
            this.viewProductDetail()
